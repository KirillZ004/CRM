                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="contact-info">
                            <?php echo !empty($employee['phone']) ? htmlspecialchars($employee['phone']) : '<span class="empty-value">Не указан</span>'; ?>
                        </td>
                        <td class="contact-info">
                            <?php echo !empty($employee['email']) ? 
                        ...
                        <td class="salary">
                            <?php echo !empty($employee['salary']) ? number_format($employee['salary'], 2, '.', ' ') . ' ₽' : '<span class="empty-value">Не указана</span>'; ?>
                        </td>
                        <td>
                            <span class="status <?php echo (!empty($employee['status']) && $employee['status'] === 'active') ? 'status-active' : 'status-inactive'; ?>">
                                <?php echo (!empty($employee['status']) && $employee['status'] === 'active') ? 'Активен' : 'Неактивен'; ?>
                            </span>
                        </td>
                        <td class="action-buttons">
                            <a href="edit_employee.php?id=<?php echo $employee['id']; ?>" class="btn-edit">Редактировать</a>
                            <a href="employees.php?delete=<?php echo $employee['id']; ?>" class="btn-delete" onclick="return confirm('Вы уверены, что хотите удалить этого сотрудника?')">Удалить</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
